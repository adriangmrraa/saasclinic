...
# Database & Persistence Master - SAAS CRM v8.0
...
- **Sincronización de Base**: Tras evolucionar el pipeline, se debe actualizar el archivo de cimiento `db/init/saas_crm_schema.sql` para nuevas instalaciones.

## 2. Multi-tenancy & Aislamiento Legal
- **Filtro tenant_id**: Todas las tablas core (`leads`, `sellers`, `seller_agenda_events`, etc.) **DEBEN** incluir y filtrar por `tenant_id` en cada consulta.
...
- `leads.social_links`: Almacena links y metadata de redes sociales.
- `sellers.working_hours`: Configuración de agenda semanal.
- `leads.history`: Registro de cambios de status y notas de seguimiento.
...
- **Conversión Lead-Cliente**: El `status` en DB (`new` → `closed_won`) dispara los protocolos de calificación.
